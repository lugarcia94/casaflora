...
replaceHtmlBody = function (environment, accountName, secureUrl, port) {
    return function (req, res, next) {
        ...
        res.end = function (chunk, encoding) {
            ...
            if (data) {
                data = data.replace(new RegExp(environment, "gi"), "vtexlocal")
                data = data.replace(new RegExp("vteximg", "gi"), "vtexlocal")
                if (secureUrl) {
                    data = data.replace(
                        new RegExp("https://" + accountName, "gi"),
                        "http://" + accountName
                    )
                }
            }
            if (port !== 80) {
                data = data.replace(
                    new RegExp("vtexlocal.com.br/", "gi"),
                    "vtexlocal.com.br:" + port + "/"
                )
            }
            res.write = write
            res.end = end
...
errorHandler = function (err, req, res, next) {
    ...
    errString =
        (ref = (ref1 = err.code) != null ? ref1.red : void 0) != null
            ? ref
            : err.toString().red
