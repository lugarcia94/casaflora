...
class BuyButton extends Component {
    ...
    setSku(sku) {
        ...
        if (
            parseInt(seller.commertialOffer.AvailableQuantity) >=
                item.quantity &&
            item.quantity > 0
        )
            valid = true
    ...
    handleQTD(qtd) {
        ...
        if (seller.commertialOffer.AvailableQuantity >= qtd)
            ...
            this.setState({
                msg:
                    "Quantidade Maxima em estoque é " +
        ...
        return (
            <div
                className={classNames({
                    "buy-button": true,
                    "buy-button--add-product": this.state.hasAddProduct,
                })}
            >
                {msg.length > 0 && <div className="buy-button__msg">{msg}</div>}
                {product.items.length > 1 && (
                    <Variants
                        onChange={this.setSku.bind(this)}
                        skus={product.items}
                    />
                )}
                <div className="buy-button__qtd">
                    <button
                        className="buy-button__less"
                        type="button"
                    ...
                    <input
                        className="buy-button__input"
                        type="text"
                        value={quantity}
                        onChange={this.update.bind(this)}
                    />
                    <button
                        className="buy-button__more"
                        type="button"
