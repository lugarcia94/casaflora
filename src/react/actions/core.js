...
export function fetchOrderForm() {
    return (dispatch) => {
        vtexjs.checkout.getOrderForm().done(function(object) {
...
export function addToCart(items) {
    return async () => {
        ...
        const itemsUpdate = items
            .filter(
                (item) =>
                    orderForm.items.filter((i) => i.id == item.id).length > 0
            )
            .map((item) => {
        ...
        items = items.filter(
            (item) => orderForm.items.filter((i) => i.id == item.id).length == 0
        )

        // Insert
        if (items.length) {
            orderForm = await vtexjs.checkout.addToCart(
                items,
                null,
                config.seller.channel
            )
        ...
        if (itemsUpdate.length) {
            orderForm = await vtexjs.checkout.updateItems(
                itemsUpdate,
                null,
                false
            )
        ...
        if (msgIndex < orderForm.messages.length) msg(orderForm.messages)
        return orderForm
    }
}
